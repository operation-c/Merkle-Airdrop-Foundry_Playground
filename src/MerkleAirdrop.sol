...
contract MerkleAirdrop is EIP712 {
    ...
    constructor(bytes32 merkleRoot, IERC20 airdropToken) EIP712("Merkle Airdrop", ".01") {
    ...
    function claim(address account, uint256 amount, bytes32[] calldata merkleProof, uint8 v, bytes32 r, bytes32 s) external {
        ...
        // calculate using the account & the amount, the hash -> lead node 
        // hashing twice to aviod collision aka second preimage attack
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(account, amount))));

        // now using "verify()" to check if our leaf matches the expected root 
        if (!MerkleProof.verify(merkleProof, i_merkleRoot, leaf)) { revert MerkleAirdrop__InvalidProof(); }

        // avoid multiple claims from single EOA
